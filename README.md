...
- **写出值得读的文章**：真实素材锚定（WebSearch，禁止编造）+ 按框架匹配的内容增强 + 准确、观点、有用、合声、好读五项编辑标准 + 11 项语言风险提示。
...
| 检查一下 / 这篇怎么样 | `wewrite-review` | 生成档案 + 最多 5 条重点改进建议 |
...
Step 4  用户风格 → 写作 → 编辑自检                    ┘
  ↓
Step 5  编辑审稿 → 标题、摘要与检索优化                 ← wewrite-review
  ↓
...
│   ├── migrate.py              # 旧状态一次性迁移
│   ├── commands/               # run / sources / diagnose / score / hotspots / search-articles / seo / stats / learn-* / exemplar / fetch-article / llm-write / similarity / build-playbook
